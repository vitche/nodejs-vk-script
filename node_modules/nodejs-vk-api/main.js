...
var VK = function(_options) {
    ...
    self.request = function(_method, _requestParams, _eventName) {
        if (self.options.mode === 'sig')
            self._sigRequest(_method, _requestParams, _eventName);
        else if (self.options.mode === 'oauth')
            self._oauthRequest(_method, _requestParams, _eventName);
        else
    ...
    self._setUpAppServerToken = function() {
        ...
        https.get(options, function(res) {
            ...
            res.on('end', function() {
                var o = JSON.parse(apiResponse);
                if (o.error) {
                    self.emit('appServerTokenNotReady', o);
